...
# Set up files
echo -e "cryptography==3.3.2 \nfuture \nrequests==2.21.0 \nparamiko \npysnmp==4.4.6 \npycryptodome \nbcrypt==3.2.2 " >>requirements.txt
echo -e "cryptography==3.3.2 \nfuture \nrequests==2.21.0 \nparamiko \npysnmp==4.4.6 \npycryptodome \nbcrypt==3.2.2 \npytest==4.4.0 \npytest-forked \npytest-xdist \nflake8 \ngit+https://github.com/threat9/threat9-test-bed.git" >>requirements-dev.txt
...
sleep 2
apt -y install autoconf automake bison bzip2 clang cmake coreutils diffutils flex gawk git grep gzip libtool make patch perl sed silversearcher-ag tar wget pkg-config python clang libcrypt libffi git openssl libsodium rust
